...
use crate::database::{Database, VttReport};
...
async fn convert_vtt_to_markdown(vtt_path: &PathBuf) -> Result<String, String> {
    let cli_path = find_vytdl_cli().await?;

    let output = Command::new(&cli_path)
        .args(["analyze", "--mode", "markdown", &vtt_path.to_string_lossy()])
        .output()
        .await
        .map_err(|e| format!("Failed to run vYtDL CLI: {}", e))?;
    ...
    if output.status.success() {
        ...
        Err(if stderr.is_empty() {
            format!("vYtDL analyze failed with code {:?}", output.status.code())
        } else {
            stderr.to_string()
        })
    }
...
fn default_output_dir() -> String {
    std::env::var("VYTDL_OUTPUT_DIR").unwrap_or_else(|_| {
        dirs::download_dir()
