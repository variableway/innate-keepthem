...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let db: Result<db::Database, ()> = tauri::async_runtime::block_on(async {
                // Try app data directory first
                match db::Database::new(&app.handle()).await {
                    Ok(db) => {
                ...
                match db::Database::new_with_path(&fallback_path).await {
                    Ok(db) => {
                        if let Err(e) = db.init().await {
            ...
            for record in incomplete {
                ...
                tauri::async_runtime::block_on(async {
                    let _ = db.update_download_status(&record.id, crate::db::DownloadStatus::Pending).await;
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::add_asset_to_session,
            commands::get_asset_groups,
            // Settings commands
            commands::get_settings,
            commands::update_settings,
            // Download commands (from vYtDL)
